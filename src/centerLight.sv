...
always_comb begin
    case (ps)
        on: if (up) begin
                ns = off;        // Bird moves up, this light turns off
                lightOn = 1'b1;
            end else begin
                ns = off;        // Bird falls down, this light turns off  
                lightOn = 1'b1;
            end
            
        off: if (below && up) begin
                ns = on;         // Bird coming from below (jumping up)
                lightOn = 1'b0;
            end else if (above && ~up) begin
                ns = on;         // Bird coming from above (falling)
                lightOn = 1'b0;
            end else begin
                ns = off;
                lightOn = 1'b0;
